...
def new_request():
    ...
    if request.method == 'GET':
        ...
        response_text = """Your test has been added in the que with <b>Id : {0}.
                        </b> <br>
...
def update_request(request_id):
    ...
    if form_data.get('request') and form_data.get('response'):
        ...
        # clean_previous_requests
        session.query(HttpRequestVerification).filter(HttpRequestVerification.request_id==request_id).delete()
        session.query(HttpResponseVerification).filter(HttpResponseVerification.request_id==request_id).delete()
